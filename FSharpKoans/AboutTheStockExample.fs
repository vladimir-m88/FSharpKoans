...
module ``about the stock example`` =
    ...
    type StockRow = { Day: string; Diff: float }
    ...
    let YouGotTheAnswerCorrect () =
        ...
        let rowWithMaxDiff x y = if x.Diff >= y.Diff then x else y
        ...
        let result =
            ...
            |> List.map (fun fields ->
                { Day = fields.[0]
                  Diff = abs (parseFloat fields.[1] - parseFloat fields.[4]) })
            |> List.reduce rowWithMaxDiff
            |> fun x -> x.Day
